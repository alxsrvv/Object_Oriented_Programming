...
bool Ñontainer::Save() const
{
	...
	if (start_node)
	{
		save_flag = true;
		ofstream save_file;
		save_file.open("Workshops_info.dat"); // make or remake file
		...
		if (save_file.is_open())
		{
			...
			for (unsigned i = 0; i < containers_length; i++)
			{
				...
				{
					save_file << 0 << endl; // for easier data processing added label to sewing workshops...
...
{
	ifstream read_file;
	read_file.open("Workshops_info.dat"); // open file

	if (read_file.is_open())
	{
		char one_two_string[10]; // char that will read labels of workshops
		read_file.getline(one_two_string, 10); // reading first label
		unsigned char one_two_num = atoi(one_two_string); // transform char to number

		Sewing_workshop* sewing_0; // temp workshops
		Furniture_workshop* furniture_1;

		while (!read_file.eof()) // while file not end yet
		{
			if (one_two_num == 0)
			{
				sewing_0 = new Sewing_workshop();
				read_file >> (*sewing_0); // read method declarated in Sewing_workshop.h
				Push(sewing_0); // add workshop to container
			}
			else if (one_two_num == 1)
			{
				furniture_1 = new Furniture_workshop();
				read_file >> (*furniture_1);// read method declarated in Furniture_workshop.h
				Push(furniture_1); // add workshop to container
			}
			read_file.getline(one_two_string, 10); // reading next workshop label
...
bool Ñontainer::Count_capacity() const
{
	if (start_node)
	{
		...
		while (tmp_node != start_node);
		if (type_flag)
			cout << "\nFull capacity of all " << type_to_count << " workshops: " << full_capacity << endl;
		else
